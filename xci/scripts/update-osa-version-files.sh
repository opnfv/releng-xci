...
sed -i -e "/^export OPENSTACK_OSA_VERSION/s@:-\"[a-z0-9]*@:-\"${1}@" \
    -e "s/\(^# HEAD of osa \).*/\1\"${OPENSTACK_OSA_VERSION}\" as of $(date +%d\.%m\.%Y)/" $releng_xci_base/config/pinned-versions

# Update the pinned bifrost version
if [[ -n ${2:-} ]]; then
  echo "Updating bifrost..."
  sed -i -e "/^export OPENSTACK_BIFROST_VERSION/s@:-\"[a-z0-9]*@:-\"${2}@" \
    -e "s/\(^# HEAD of bifrost \).*/\1\"${OPENSTACK_OSA_VERSION}\" as of $(date +%d\.%m\.%Y)/" $releng_xci_base/config/pinned-versions
  # Get ironic shas
  for ironic in ironic_git_url ironic_client_git_url ironic_inspector_git_url ironic_inspector_client_git_url; do
    ...
    sed -i -e "/^export BIFROST_${ironic^^}_VERSION/s@:-\"[a-z0-9]*@:-\"${ironic_sha}@" \
      -e "s/\(^# HEAD of ${ironic/_/-} \).*/\1\"${OPENSTACK_OSA_VERSION}\" as of $(date +%d\.%m\.%Y)/" $releng_xci_base/config/pinned-versions
  done
fi
