...
echo "Preparying XCI environment..."
# Start with good dns
eval $vm_ssh@$_ip "sudo bach -c 'echo nameserver 8.8.8.8 > /etc/resolv.conf'"
eval $vm_ssh@$_ip "sudo bach -c 'echo nameserver 8.8.4.4 >> /etc/resolv.conf'"

rsync -a -e "$vm_ssh" ${BASE_PATH} $_ip:~/

echo "Running XCI..."
eval $vm_ssh "cd releng-xci/xci && ./xci-deploy.sh"
