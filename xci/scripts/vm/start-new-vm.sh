...
sudo sed -i "/^${NAME}_xci_vm.*${_ip}$/d" /etc/hosts
sudo bash -c "echo '${NAME}_xci_vm ${_ip} >> /etc/hosts'"
...
IdentityFile ${BASE_PATH}/xci/scripts/vm/id_rsa_for_dib
TCPKeepAlive yes
# Hardcoded ip for opnfv VM
Hostname 192.168.122.2
ProxyCommand ssh $(echo %h | sed 's/_opnfv//') 'nc %h %p'
EOF
