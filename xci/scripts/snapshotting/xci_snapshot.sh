...
function export_domains(){
  ...
  rm -rf $EXPORT_PATH
  mkdir -p $EXPORT_PATH
  sudo cp $XCI_PATH/xci/installer/$INSTALLER_TYPE/files/$XCI_FLAVOR/inventory* $EXPORT_PATH/inventory
  for node in $xci_vms;do
...
function import_domains(){
  ...
  popd
  source $INSTALLER_TYPE/after_import_hook.sh $untar_dir/inventory
  rm -rf $untar_dir
  import_time=$SECONDS
...
if [ -z "$XCI_FLAVOR" ]; then
    echo "Set the XCI_FLAVOR environment variable. Exiting."
    exit 1
fi
if [ -z "$INSTALLER_TYPE" ]; then
    echo "Set the INSTALLER_TYPE environment variable. Exiting."
    exit 1
fi
