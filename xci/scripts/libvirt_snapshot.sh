...
NODE_CONFIG=$HOME/.snapshot/node_names
...
function get_node_names(){
  ...
  if [[ "$node_names"x == x ]];then
    if [ -e $NODE_CONFIG ];then
      echo "Using $NODE_CONFIG for the node names"
      node_names=$(cat $NODE_CONFIG)
    else
...
if [[ "$SNAPSHOT" != true ]];then
  # if SNAPSHOT is true we are coming from the development/deploy.sh script
  parse_arguments $@
fi
