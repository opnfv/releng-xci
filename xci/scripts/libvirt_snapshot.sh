...
NODE_CONFIG=`sudo virsh list --name | grep 'opnfv\|controller*\|compute*\|master*\|node*\|kube-node\|kube-master'`
...
##############################################################################
# sets the list of VMs present in the XCI deployment
##############################################################################
...
function export_domains(){
  ...
  pushd $EXPORT_PATH
  sudo tar --absolute-names -czvf $EXPORT_PATH/xci_export_$DATE.tar.gz $disks $node_names nodes
  popd
  virsh_resume
}
