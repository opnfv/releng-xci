...
if ! ${_XCI_ENV_IS_PREPARED_:=false}; then
    echo "This script is not meant to be used on its own as it's pretty"
    echo "destructive to the running host. Please run './xci-deploy.sh'"
    echo "instead which invoke this script in a secure environment."
    exit 1
fi
