...
#-------------------------------------------------------------------------------
# Copy the scenario role
#-------------------------------------------------------------------------------
if [[ $OPNFV_SCENARIO == "os-odl-sfc" ]]; then
  git clone https://gerrit.opnfv.org/gerrit/sfc /tmp/sfc
  pushd /tmp/sfc
#  git fetch https://gerrit.opnfv.org/gerrit/sfc refs/changes/65/43165/3 && git checkout FETCH_HEAD
  git fetch https://gerrit.opnfv.org/gerrit/sfc refs/changes/07/43607/3 && git checkout FETCH_HEAD
  popd
  cp -r /tmp/sfc/scenarios/os-odl-sfc/role/os-odl-sfc $XCI_PATH/playbooks/roles
  sudo rm -r /tmp/sfc
fi
