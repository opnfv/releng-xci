...
class XCIInventory(dict):
    def __init__(self, *args, **kw):
        ...
        self.inventory['_meta']['groupvars'] = {}
    ...
    def read_pdf_idf(self):
        ...
        valid_host = (host for host in idf['xci'][self.installer]['nodes_roles'] if host in idf['xci']['flavors'][self.flavor] and host != 'opnfv')
    ...
    def add_groupvar(self, group, param, value):
        if group not in self.groupvars():
            self.inventory['_meta']['groupvars'][group] = {}
        self.inventory['_meta']['groupvars'][group].update({param: value})

    def hostvars(self):
        return iter(self.inventory['_meta']['hostvars'].keys())

    def groupvars(self):
        return iter(self.inventory['_meta']['groupvars'].keys())
