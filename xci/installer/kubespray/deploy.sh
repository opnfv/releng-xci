...
# Configurate the kubernetes authentication in opnfv host.
...
DASHBOARD_SERVICE=$(ssh root@$OPNFV_HOST_IP "kubectl get service -n kube-system |grep kubernetes-dashboard |awk '{ print  $3}'")
DASHBOARD_PORT=$(echo ${DASHBOARD_SERVICE} | awk '{print $5}' |awk -F "[:/]" '{print $2}')
KUBER_SERVER_URL=$(ssh root@$OPNFV_HOST_IP "grep -r server ~/.kube/admin.conf")
...
echo 'Info: dashboard user:'
echo $USER_CSV |awk -F ',' '{print $2}'
echo 'Info: dashboard password:'
echo $USER_CSV |awk -F ',' '{print $1}'
