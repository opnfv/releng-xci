...
if [[ ${XCI_DISTRO,,} == centos || ${XCI_DISTRO,,} == opensuse ]]; then
    echo ""
    echo "Error: Sorry, only Ubuntu hosts is supported for now!"
    echo "Error: CentOS 7 and opensuse support is still working in progress."
    echo ""
    exit 1
fi
...
# - copies flavor files to be used by kubesptay
# - install packages required by kubesptay
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# This playbook is only run for the all flavors except aio since aio is configuredby the configure-opnfvhost.yml
# This playbook
...
echo "Info: Using kubespray to deploy the kubernetes"
echo "-----------------------------------------------------------------------"
